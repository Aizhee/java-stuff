...
public class NewtonRaphsonSolver extends RootCalculator {
	...
	public String solveModifiedRoot(String expression, double x0, double tolerance) {
        ...
        for (int i = 0; i < fDoublePrime.toString().length(); i++) {
            formattedString.append(fDoublePrime.toString().charAt(i));
            if ((i + 1) % 100 == 0) {
	    ...
	    while (iterations < maxIterations && Math.abs(solve.evaluateExpression(expression.replaceAll("x", String.valueOf(xn)))) > tolerance) {
	        double xi = xn;
	        double f = solve.evaluateExpression(expression.replaceAll("x", String.valueOf(xn)));
	        ...
	        System.out.println(expression.replaceAll("x", String.valueOf(xn)));
	        System.out.println(fPrime.toString().replaceAll("x", String.valueOf(xn)));
	        System.out.println(fDoublePrime.toString().replaceAll("x", String.valueOf(xn)));
